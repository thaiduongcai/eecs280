...
vector<double> mean_diff_sampling_distribution(
  ...
  vector<double> resample_A = {};
  vector<double> resample_B = {};

  for (int i = 1; i <= 1000; ++i)
  {
    resample_A = bootstrap_resample(data_A, i);
    resample_B = bootstrap_resample(data_B, i);
    result.push_back((mean(resample_A))-(mean(resample_B)));
  }
...
pair<double, double> confidence_interval(vector<double> v, double width) {
  ...
  lower = percentile(v, 0.5-(width/2));
  upper = percentile(v, 0.5+(width/2));
